...
// If editing, get current article data
if ($id) {
...
// Get all members - use 'id' as primary key per schema
$sql     = "SELECT id, forename, surname FROM member;";
$authors = pdo($pdo, $sql)->fetchAll();

// Get all categories - use 'id' as primary key per schema
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $errors['image_file'] = (!empty($_FILES) and $_FILES['image']['error'] === 1) ? 'File too big ' : '';
    ...
    if ($temp and $_FILES['image']['error'] === 0) {
        ...
        $errors['image_file'] .= ($_FILES['image']['size'] <= $max_size)
            ? '' : 'File too big. ';
        $errors['image_alt'] = (is_text($article['image_alt'], 1, 1000))
            ? '' : 'Alt text must be 1-1000 characters.';
    ...
    $article['member_id']   = (int) $_POST['member_id'];
    ...
    // Part C: Update database if valid
    if ($invalid) {
