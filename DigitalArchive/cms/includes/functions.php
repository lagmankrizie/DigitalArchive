...
{
    $extension = strtolower(pathinfo($filename, PATHINFO_EXTENSION));  // Get extension
    $basename  = substr(pathinfo($filename, PATHINFO_FILENAME), 0, 50); // Get base name (max 50 chars)
    $basename  = preg_replace('/[^A-Za-z0-9-]/', '-', $basename);     // Replace non-alphanumeric with dash
    $candidate = $basename . '.' . $extension;                        // Build candidate filename
    $counter   = 1;                                                   // Counter for duplicates
    while (file_exists($uploads . $candidate)) {                      // While filename exists
        $candidate = $basename . '-' . $counter . '.' . $extension;  // Add counter to filename
        $counter++;                                                   // Increment counter
    }
    return $candidate;                                                // Return unique filename
}
